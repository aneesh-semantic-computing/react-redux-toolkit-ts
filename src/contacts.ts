...
export async function getContact(id: number): Promise<ContactType> {
  await fakeNetwork(`contact:${id}`);
  let contacts = await localforage.getItem<any>("contacts");
  let contact = contacts.find((contact:any) => contact.id === id) as ContactType;
  return contact ?? null;
}
...
function set(contacts:ContactType[]|null) {
